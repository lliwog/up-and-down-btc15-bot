# Strategy behavior (S2: Mid Game):
# - Activates during the middle window (5–10 minutes left) with a stricter TA
#   threshold and a discounted token price requirement.
...
class MidGameStrategy(BaseStrategy):
    name = StrategyName.MID_GAME

    def should_enter(self, signal: TASignal) -> bool:
        if signal.side is None:
            return False
        return (
            5 <= signal.timeLeftMin <= 10
            and signal.ta_score > 0.70
            and signal.token_price < 50
        )
